...
if (image1_app[cnt_vect][cnt_dim]>max){ //image_sobel
max = image1_app[cnt_vect][cnt_dim]; //image_sobel
}
}}
...
for(cnt_dim=0;cnt_dim<W1;cnt_dim++){//W - image_sobel
 ...
 image1_app[cnt_vect][cnt_dim] = image1_app[cnt_vect][cnt_dim] ^ (1 <<(rand()%8));
...
fd = fopen("eagle9x_qbin8.h", "w+");
fprintf(fd,"#define H1 %d\n",H1); //H
fprintf(fd,"#define W1 %d\n",W1); //W
fprintf(fd,"int image1_app[%d][%d] = {\n",H1,W1);//H,W
for(cnt_vect=0;cnt_vect<H1;cnt_vect++){
fprintf(fd,"{");
for(cnt_dim=0;cnt_dim<W1;cnt_dim++){
//fprintf(fd,"%d,",image_sobel[cnt_vect][cnt_dim]);
fprintf(fd,"%d,",image1_app[cnt_vect][cnt_dim]);
}
fprintf(fd,"},\n");
}
fprintf(fd,"};");
