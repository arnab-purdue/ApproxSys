...
int runCascadeClassifier( myCascade* _cascade, MyPoint pt, int start_stage )
{
  ...
  int start_stage_offset = 0;//Arnab added
  for( i = start_stage+start_stage_offset; i < cascade->n_stages; i++ )
 // for( i = start_stage+start_stage_offset; i < cascade->n_stages; i=i+1 )
    {
      ...
      for( j = 0; j < stages_array[i]; j++ )
	{
      ...
      //if( stage_sum < 0.4*stages_thresh_array[i] ){ //orig
      if (stages_thresh_array[i]>0){
      if( stage_sum < (0.4+fuzz)*stages_thresh_array[i] ){ //orig
	return -i;
...
//Arnab added
//printf("x2 = %d, y2 = %d\n",x2,y2);
//step is a tunable knob  
for( x = 0; x <= x2; x += stepx )
    for( y = y1; y <= y2; y += stepy )
   //for( y = y1; y <= y2; y += step*2 )
      {
	p.x = x;
	p.y = y;
